...
def elastic_health_check(request: Request) -> Response:
    ...
    if es.ping():
        ...
        return Response(
            {"status": "not connected"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            name="number", description="Invoice number", required=True, type=str
        ),
        OpenApiParameter(
            name="sort_by",
            description="Field to sorting(e.g. created_at)",
            required=False,
            type=str,
        ),
        OpenApiParameter(
            name="order",
            description="Sorting direction: asc lub desc",
            required=False,
            type=str,
        ),
        OpenApiParameter(
...
class SearchByNumberAPIView(InvoiceSearchAPIView):
    def get(self, request: Request) -> Response:
        ...
        results = InvoiceDocument.search().query(
            "match", number={"query": query, "fuzziness": "AUTO"}
        )
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            ...
            required=True,
            location=OpenApiParameter.QUERY,
            description="Comma-separated list of date fields to filter (e.g. 'issue_date,payment_date').",
        ),
        OpenApiParameter(
            name="date_from",
            type=str,
...
class SearchByDatesAPIView(InvoiceSearchAPIView):
    def get(self, request: Request) -> Response:
        ...
        if not validate_sort_fields(
            date_names, InvoiceDocument.ELASTIC_ALLOWED_SEARCH_FIELDS
        ):
            return Response(
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            ...
            required=True,
            location=OpenApiParameter.QUERY,
            description="Phrase to search in description or product name.",
        ),
    ],
    responses={
        200: InvoiceElasticsearchSerializer(many=True),
        ...
        results = InvoiceDocument.search().query(
            ...
            should=[
                ...
                Q(
                    "nested",
                    path="products",
                    query=Q("match", **{"products.name": phrase}),
                ),
            ],

...
from ..exceptions.mail_notification_exceptions import NotificationMailMissingFieldException
...
class MailNotify(Notify):
    ...
    def _get_valid_message_fields(self, fields_to_check: dict[str, str]) -> dict[str, str]:
        missing = [field for field in self._valid_message_fields if field not in fields_to_check]
        ...
        return {key: fields_to_check[key] for key in self._valid_message_fields if key in fields_to_check}

    def _generate_message(self, message_data: dict[str, str]) -> str:
        return message_data['message']

    def send(self, mails: list[str], message_data: dict[str, str], subject: str = "Mail Notification") -> None:
